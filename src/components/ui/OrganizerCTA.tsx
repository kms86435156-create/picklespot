...
export default function OrganizerCTA({ variant = "sidebar", context = "general" }: Props) {
    ...
    return (
      <div className="bg-gradient-to-r from-brand-cyan/10 via-brand-cyan/5 to-brand-red/5 border border-brand-cyan/20 rounded-lg p-6 md:p-8">
        <div className="flex flex-col md:flex-row items-center gap-5">
          ...
          <Link href="/for-clubs" className="shrink-0 inline-flex items-center gap-2 px-6 py-3 bg-brand-cyan text-dark font-bold text-sm rounded hover:bg-brand-cyan/90 transition-colors">
  ...
  if (variant === "sidebar") {
    return (
      <div className="bg-brand-cyan/5 border border-brand-cyan/20 rounded-lg p-4">
        ...
        <Link href="/for-clubs" className="inline-flex items-center gap-1 text-xs text-brand-cyan hover:underline font-bold">
          무료 사전등록 <ArrowRight className="w-3 h-3" />
        </Link>
      </div>
    );
  }
  ...
  return (
    <div className="flex items-center gap-3 bg-surface border border-ui-border rounded-lg p-4 hover:border-brand-cyan/20 transition-colors">
      ...
      <Link href="/for-clubs" className="shrink-0 text-xs text-brand-cyan hover:underline font-bold flex items-center gap-1">
