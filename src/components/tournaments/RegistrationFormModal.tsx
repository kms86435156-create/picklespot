...
const DIVISIONS = ["남자복식", "여자복식", "혼합복식", "남자단식", "여자단식"];
const LEVELS = ["입문", "D", "C", "B", "A", "S/Pro"];
...
export default function RegistrationFormModal({ tournament: t, onClose }: Props) {
  ...
  // 대회 종별 파싱
  const divisions = (t.eventTypes || t.divisions || "").split(/[,，]/).map((s: string) => s.trim()).filter(Boolean);
      ...
      <div className="relative w-full max-w-lg bg-surface border border-ui-border rounded-lg overflow-hidden my-4">
        ...
        <div className="sticky top-0 bg-surface border-b border-ui-border px-5 py-4 flex items-center justify-between z-10">
          <div>
            ...
            <h2 className="font-bold text-lg text-white">대회 신청</h2>
          </div>
          ...
          <form onSubmit={handleSubmit} className="p-5 space-y-4">
            ...
            <div className="bg-dark/50 border border-ui-border rounded-lg p-4">
              ...
              <div className="text-xs text-text-muted space-y-0.5">
                <p>{t.startDate} · {t.venueName || t.location || "장소 확인중"}</p>
                <p>참가비: ₩{(t.entryFee || 0).toLocaleString()} · 정원: {t.maxParticipants || "제한없음"}명</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input type="text" required value={form.playerName} onChange={e => set("playerName", e.target.value)}
                  placeholder="홍길동"
              ...
              <div>
                ...
                <input type="tel" required value={form.playerPhone} onChange={e => set("playerPhone", e.target.value)}
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <label className="block text-[11px] text-text-muted mb-1">종별</label>
                <select value={form.division} onChange={e => set("division", e.target.value)}
                  className="w-full px-3 py-2 bg-dark border border-ui-border rounded text-sm text-white focus:border-brand-cyan focus:outline-none">
                  <option value="">선택</option>
                  {(divisions.length > 0 ? divisions : DIVISIONS).map((d: string) => (
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-[11px] text-text-muted mb-1">파트너명 <span className="text-text-muted/50">(복식)</span></label>
                <input type="text" value={form.partnerName} onChange={e => set("partnerName", e.target.value)}
                  placeholder="파트너 이름"
                  className="w-full px-3 py-2 bg-dark border border-ui-border rounded text-sm text-white placeholder:text-text-muted/50 focus:border-brand-cyan focus:outline-none" />
              </div>
              <div>
                <label className="block text-[11px] text-text-muted mb-1">소속 동호회 <span className="text-text-muted/50">(선택)</span></label>
                <input type="text" value={form.clubName} onChange={e => set("clubName", e.target.value)}
                  placeholder="동호회명"
                  className="w-full px-3 py-2 bg-dark border border-ui-border rounded text-sm text-white placeholder:text-text-muted/50 focus:border-brand-cyan focus:outline-none" />
              </div>
            </div>
            ...
            <div>
              <label className="block text-[11px] text-text-muted mb-1">실력 레벨 <span className="text-text-muted/50">(선택)</span></label>
            ...
            <div>
              <label className="block text-[11px] text-text-muted mb-1">메모 <span className="text-text-muted/50">(선택)</span></label>
              <input type="text" value={form.memo} onChange={e => set("memo", e.target.value)}
                placeholder="알레르기, 식사 포함 여부 등"
                className="w-full px-3 py-2 bg-dark border border-ui-border rounded text-sm text-white placeholder:text-text-muted/50 focus:border-brand-cyan focus:outline-none" />
            </div>
        ...
        {step === "success" && (
          <div className="p-8 text-center">
            ...
            <h3 className="text-xl font-black text-white mb-2">
              {waitlisted ? "대기자 등록 완료!" : "신청 완료!"}
            </h3>
            <p className="text-sm text-text-muted mb-4">
              {waitlisted
                ? "현재 정원이 마감되어 대기자로 등록되었습니다. 빈자리 발생 시 연락드리겠습니다."
                : "관리자 확인 후 참가가 확정됩니다. 확정 시 연락드리겠습니다."
              }
            </p>
            <div className="bg-dark/50 border border-ui-border rounded-lg p-4 text-left space-y-1.5 text-sm mb-6">
              <div className="flex justify-between"><span className="text-text-muted">대회</span><span className="text-white font-medium">{t.title}</span></div>
              ...
              {form.division && <div className="flex justify-between"><span className="text-text-muted">종별</span><span className="text-white">{form.division}</span></div>}
              <div className="flex justify-between"><span className="text-text-muted">상태</span><span className={waitlisted ? "text-yellow-400" : "text-brand-cyan"}>{waitlisted ? "대기" : "승인대기"}</span></div>
            </div>
            <button onClick={onClose} className="w-full py-3 bg-brand-cyan/10 border border-brand-cyan/20 text-brand-cyan font-bold text-sm rounded hover:bg-brand-cyan/20 transition-colors">
              닫기
            </button>
          </div>
        )}
          ...
          <div className="p-8 text-center">
            ...
            <div className="flex flex-col gap-2">
              <button onClick={() => { setStep("form"); setError(""); }}
                className="w-full py-3 bg-brand-cyan text-dark font-bold text-sm rounded">다시 시도</button>
