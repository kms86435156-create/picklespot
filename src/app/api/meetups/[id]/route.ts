...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (!meetup) return NextResponse.json({ error: "찾을 수 없습니다" }, { status: 404 });
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (meetup.hostId !== session.id) {
    return NextResponse.json({ error: "주최자만 수정할 수 있습니다." }, { status: 403 });
  }

  const body = await req.json();
  const allowed = [
    "title", "date", "startTime", "endTime", "venueName", "venueAddress",
    "region", "maxPlayers", "skillLevel", "fee", "description",
    "isBeginnerFriendly", "status",
  ];
